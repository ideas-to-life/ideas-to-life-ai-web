...
- pipeline-based execution (multi-step orchestration)
- concurrency and parallel execution patterns (fan-out / fan-in, batching)
...
6) Identify distributed and behavioural patterns:
   - patterns that emerge across multiple files or components
   - coordination between orchestrator, steps, and shared state
   - execution behaviours (e.g., pipelines, parallel execution, batching)
   - do not require a single class or function to define the pattern
   - treat orchestration + state + flow as a single unit of analysis

## Final check (before output)

Ensure the following have been considered:

- Is there a shared execution context passed across steps?
- Is there a pipeline or multi-step execution model?
- Is there any form of parallel or concurrent execution?
- Does orchestration define execution behaviour explicitly?

If yes, verify these are captured as patterns.

## Reporting rules
- Prioritise patterns that influence agent behaviour and control.
- Merge similar patterns aggressively.
- Do NOT merge patterns if they represent different architectural dimensions (e.g., state management vs execution behaviour vs orchestration)
...
- Evidence may span multiple files when the pattern is distributed
- A valid pattern may be evidenced by the interaction between components, not a single symbol

## Pattern detection rule (critical)

If a behaviour:
- spans multiple components (e.g., orchestrator + steps + shared state)
- defines how execution flows through the system
- is repeatable and central to system behaviour

Then it MUST be considered a pattern, even if no single class or function encapsulates it.
